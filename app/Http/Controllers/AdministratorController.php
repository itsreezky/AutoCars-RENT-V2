...

{
    ...
    public function administratorusers(Request $request) // Home Page
    {
        ...
        public function administratorowners(Request $request) // Home Page
        {
            ...
            public function administratorownerspending(Request $request) // Home Page
        {

            //get all owners
            $allownerspending = Owners::get();
            //count total owners
            $ownerspendingCount = count($allownerspending);

                //save data to view
                return view(
                    'Administrator.data-owners-pending',
                    compact('allownerspending', 'ownerspendingCount')
                );
            }
            public function ownerspendingupdate(Request $request, $id)

    {
        ...
        toast('Owners has verified now','success','top-end')->autoClose(7000);
        return back();
    }
